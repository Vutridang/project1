...
class ProductController extends Controller
{
    ...
    public function search(Request $request){
        ...
        $products = Product::select(['products.*','categorys.name as category_name'])->join('categorys','categorys.id','=','products.category_id');
    ...
    public function store(ProductRequest $request){
        ...
        try{
           ...
           $product->user_id = auth()->user()->id;//user_id tạo sản phẩm phải lấy user đăng nhập
           $res = $product->save();
           $id = $product->id;//$id dùng để insert vào trong table images
           $data = [];
