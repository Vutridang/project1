					...
					<ul class="nav nav-tabs" id="myTab" role="tablist">
						@foreach($categorys as $key => $category)
						    <li class="nav-item item-category" data-id="{{$category->id}}">
						    	<a class="nav-link {{$category->id == Request::input('id') ? 'active' : ''}}" data-toggle="tab" data-id="{{$category->id}}" href="#{{$category->slug}}" role="tab">{{$category->name}}</a>
						    </li>
						@endforeach
					</ul>
								   ...
								   <div class="col-xl-3 col-lg-4 col-md-4 col-12">
								   	    <div class="single-product">
								   	    	...
								   	    	<div class="product-content">
					                             <h3>
						                             <a href="{{route('home.product.detail', [$prod->slug])}}">{{$prod->name}}</a>
					                             </h3>
