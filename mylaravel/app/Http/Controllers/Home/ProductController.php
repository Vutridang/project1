...
class ProductController extends Controller
{
    ...
    $products = Product::select(['products.*','categorys.name as category_name'])->join('categorys','categorys.id','=','products.category_id')->where(function($query) use($idCategory){
        $query->where('categorys.id',$idCategory)->orWhere('categorys.parent_id', $idCategory);
    })->with('images');
    if($request->has('q')){
    ...
    return view('Home.index', ['categorys' => $categorys, 'products' => $products]);
    }
